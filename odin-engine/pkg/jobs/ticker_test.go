...
func TestSortQueue(t *testing.T) {
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v.get() ", testCase.A), func(t *testing.T) {
			...
			<-channel
			for inc := range unsorted.Items {
				if actual[inc].Schedule[0] != testCase.Expected[inc].Schedule[0] {
					t.Errorf("TestSortQueue %d failed - expected: '%v' got: '%v'", i+1, actual[inc].Schedule, testCase.Expected[inc].Schedule)
				}
			}
		})
	}
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v.get() ", testCase.A), func(t *testing.T) {
			...
			if actual != testCase.Expected {
				t.Errorf("TestCheckHead %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)
			}
		})
	}
}
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v.get() ", testCase.A), func(t *testing.T) {
			...
			if !reflect.DeepEqual(actual, testCase.Expected) {
				t.Errorf("TestGroupItems %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)
			}
		})
	}
}
...
func TestCronToSeconds(t *testing.T) {
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v.get() ", testCase.A), func(t *testing.T) {
			...
			if result != expected {
				t.Errorf("TestCronToSeconds %d failed - expected: '%v' got: '%v'", i+1, result, expected)
			}
		})
	}
}
