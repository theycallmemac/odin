...
func TestExecutorYaml(t *testing.T) {
    cases := []struct {Name string; A string; Expected string} {
        ...
        {"parse an non-existant yaml file", "./testConfigs/false.yml", "Failed to read file."},
    }
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("%v.get() ", testCase.A), func(t *testing.T) {
            actual, _ := ExecutorYaml(testCase.A)
            if (actual != testCase.Expected) {t.Errorf("TestExecutorYaml %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)}
        })
    }
}
...
func TestSchedulerYaml(t *testing.T) {
    ...
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("%v.get() ", testCase.A), func(t *testing.T) {
            ...
            if (actual != testCase.Expected) {t.Errorf("TestSchedulerYaml %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)}
        })
    }
}
...
func TestNotEmpty(t *testing.T) {
    ...
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("read(%s) ", testCase.A), func(t *testing.T) {
            ...
            if (actual != testCase.Expected) {t.Errorf("TestNotEmpty %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)}
        })
    }
}
...
func TestReadFileBytes(t *testing.T) {
    cases := []struct {Name string; A string; Expected []byte} {
        ...
        {"read a non-existant yaml file", "testConfigs/false.yml", b},
    }
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("read(%s) ", testCase.A), func(t *testing.T) {
            actual := ReadFileBytes(testCase.A)
            if (reflect.TypeOf(actual) != reflect.TypeOf(testCase.Expected)) {t.Errorf("TestReadFileBytes %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)}
        })
    }
}
...
func TestUnmarshalYaml(t *testing.T) {
    ...
    cases := []struct {Name string; A []byte; Expected types.EngineConfig} {
    ...
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("%v.parse() ", testCase.A), func(t *testing.T) {
            ...
            if (actual != testCase.Expected) {t.Errorf("TestUnmarshalYaml %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)}
        })
    }
}
