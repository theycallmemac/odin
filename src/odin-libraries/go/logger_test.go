...
func TestLog(t *testing.T) {
    cases := []struct {Name string; A, B, C, D string; Expected bool;} {
        {"watch operation", "watch", "watch this variable", "1000", "fake_id", true},
        {"condition operation", "condition", "evaluate and watch this condition", "1001", "fake_id", true},
        {"result operation", "result", "exit on this result", "1002", "fake_id", true},
    }
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("Log(%s, %s, %s, %s) ", testCase.A, testCase.B, testCase.C, testCase.D), func(t *testing.T) {
            actual := Log(testCase.A, testCase.B, testCase.C, testCase.D)
...
func TestFindAndInsert(t *testing.T) {
    ...
    for i, testCase := range cases {
        t.Run(fmt.Sprintf("FindAndInsert(%s, %s, %s, %s) ", testCase.A, testCase.B, testCase.C, testCase.D), func(t *testing.T) {
            actual := FindAndInsert(testCase.A, testCase.B, testCase.C, testCase.D)
