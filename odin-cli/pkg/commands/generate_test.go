	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v ", testCase.Name), func(t *testing.T) {
			...
			if actual != testCase.Expected {
				t.Errorf("TestGenerateFiles %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)
			}
		})
	}
}
...
func TestGenerateCreate(t *testing.T) {
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v ", testCase.Name), func(t *testing.T) {
			...
			if actual != testCase.Expected {
				t.Errorf("TestGenerateCreate %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)
			}
		})
	}
}
