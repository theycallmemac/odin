	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v ", testCase.Name), func(t *testing.T) {
			...
			if actual != testCase.Expected {
				t.Errorf("TestExecuteYaml %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)
			}
		})
	}
}
...
func TestExecuteReadFile(t *testing.T) {
	...
	for i, testCase := range cases {
		t.Run(fmt.Sprintf("%v ", testCase.Name), func(t *testing.T) {
			...
			if actual != testCase.Expected {
				t.Errorf("TestExecuteReadFile %d failed - expected: '%v' got: '%v'", i+1, actual, testCase.Expected)
			}
		})
	}
}
